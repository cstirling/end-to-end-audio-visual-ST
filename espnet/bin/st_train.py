...
def get_parser(parser=None, required=True):
    ...
    parser.add_argument('--device', type=str, default="cuda:0",
                        help='device for decoding: cuda/cpu')
    parser.add_argument('--multimodal', default=False, action='store_true',
                        help='speech + video translation')
    return parser
...
def main(cmd_args):
    ...
    if args.backend == "pytorch":
        if args.multimodal == True:
            from espnet.st.pytorch_backend.multimodal_st import train
        else:
