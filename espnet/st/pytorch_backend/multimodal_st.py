...
class MultimodalConverter(object):
    """Multimodal batch converter for Pytorch.
    ...
    def __init__(self, subsampling_factor=1, dtype=torch.float32):
        """Construct a MultimodalConverter object."""
    ...
    def __call__(self, batch, device=torch.device('cpu')):
        ...
        visual_xs = pad_list([torch.from_numpy(x).float().unsqueeze(0) for x in visual_xs], 0).to(device, dtype=self.dtype)
        ...
        return xs_pad, ilens, visual_xs, ys_pad, ys_pad_asr
...
def train(args):
    ...
    # Setup a converter
    converter = MultimodalConverter(subsampling_factor=subsampling_factor, dtype=dtype)
