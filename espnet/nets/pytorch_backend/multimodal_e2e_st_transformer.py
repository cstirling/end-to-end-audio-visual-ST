...
from espnet.nets.pytorch_backend.transformer.multimodal_decoder import MultimodalDecoder
from espnet.nets.pytorch_backend.transformer.multimodal_attention import MultimodalMultiHeadedAttention
...
class E2E(STInterface, torch.nn.Module):
    ...
    def add_arguments(parser):
        ...
        # Custom
        group.add_argument('--visual-dim', default=2048, type=int,
                           help='dimension of visual feature')
        group.add_argument('--layer-fusion', default=False, type=strtobool,
                           help='whether use layer fusion')
        group.add_argument("--encoder-type", type=str, default="all_add",
                           choices=["all_add", "proportion_add", "vat"],
                           help='Encoder type')
        group.add_argument('--vbs', default=False, type=strtobool,
                           help='whether use Visual Beginning-of-Sentence')
        group.add_argument('--noise-type', type=str, default="none", 
    ...
    def forward(self, xs_pad, ilens, visual_xs, ys_pad, ys_pad_src):
        ...
        pred_pad, pred_mask = self.decoder(ys_in_pad, ys_mask, hs_pad, hs_mask, vbs)
    ...
    def translate(self, x, visual_x, trans_args, char_list=None, rnnlm=None, use_jit=False):
        ...
        for i in six.moves.range(maxlen):
            ...
            for hyp in hyps:
                ...
                if use_jit:
                    ...
                    local_att_scores = traced_decoder(ys, ys_mask, enc_output, vbs)[0]
                else:
                    local_att_scores = self.decoder.forward_one_step(ys, ys_mask, enc_output, vbs)[0]
    ...
    def calculate_all_attentions(self, xs_pad, ilens, visual_xs, ys_pad, ys_pad_src):
        ...
        for name, m in self.named_modules():
            if isinstance(m, MultimodalMultiHeadedAttention) and m.attn is not None:  # skip MHA for submodules
