...
class EncoderConv2dMultiLength(torch.nn.Module):
    """VGG-like module
    ...
    def __init__(self, idim, dim):
        super(EncoderConv2d, self).__init__()
        # CNN layer (VGG motivated)
        self.conv1_1_g1 = torch.nn.Conv2d(1, 64, 3, stride=1, padding=1)
        self.norm1_1_g1 = torch.nn.LayerNorm([64, idim])
        self.conv1_2_g1 = torch.nn.Conv2d(64, 64, 3, stride=1, padding=1)
        self.norm1_2_g1 = torch.nn.LayerNorm([64, idim])
        idim = int(math.ceil(float(idim) / 2))

        self.conv2_1_g1 = torch.nn.Conv2d(64, 128, 3, stride=1, padding=1)
        self.norm2_1_g1 = torch.nn.LayerNorm([128, idim])
        self.conv2_2_g1 = torch.nn.Conv2d(128, 128, 3, stride=1, padding=1)
        self.norm2_2_g1 = torch.nn.LayerNorm([128, idim])

        self.conv1_1_g2 = torch.nn.Conv2d(1, 64, 3, stride=1, padding=1)
        self.norm1_1_g2 = torch.nn.LayerNorm([64, idim])
        self.conv1_2_g2 = torch.nn.Conv2d(64, 64, 3, stride=1, padding=1)
        self.norm1_2_g2 = torch.nn.LayerNorm([64, idim])
        idim = int(math.ceil(float(idim) / 2))

        self.conv2_1_g2 = torch.nn.Conv2d(64, 128, 3, stride=1, padding=1)
        self.norm2_1_g2 = torch.nn.LayerNorm([128, idim])
        self.conv2_2_g2 = torch.nn.Conv2d(128, 128, 3, stride=1, padding=1)
        self.norm2_2_g2 = torch.nn.LayerNorm([128, idim])

        idim = int(math.ceil(float(idim) / 2))

        self.out = torch.nn.Linear(idim * 128, dim)

    def g1forward(self,xs_pad,ilens):
        '''
                :param torch.Tensor xs_pad: batch of padded input sequences (B, Tmax, D)
                :param torch.Tensor ilens: batch of input length (B)
                :return: batch of padded hidden state sequences (B, Tmax // 4, 128)
        ...
        mask = ~make_pad_mask(ilens).unsqueeze(1).unsqueeze(-1)
        xs_pad = self.conv1_1_g1(xs_pad)
        xs_pad = self.norm1_1_g1(xs_pad.transpose(1, 2)).transpose(1, 2)
        xs_pad = F.relu(xs_pad)
        xs_pad = xs_pad.masked_fill(mask, 0.0)

        xs_pad = self.conv1_2_g1(xs_pad).masked_fill(mask, 0.0)
        xs_pad = self.norm1_2_g1(xs_pad.transpose(1, 2)).transpose(1, 2)
        xs_pad = F.relu(xs_pad)
        xs_pad = xs_pad.masked_fill(mask, 0.0)
        ...
        mask = ~make_pad_mask(ilens).unsqueeze(1).unsqueeze(-1)
        xs_pad = self.conv2_1_g1(xs_pad)
        xs_pad = self.norm2_1_g1(xs_pad.transpose(1, 2)).transpose(1, 2)
        xs_pad = F.relu(xs_pad)
        xs_pad = xs_pad.masked_fill(mask, 0.0)

        xs_pad = self.conv2_2_g1(xs_pad)
        xs_pad = self.norm2_2_g1(xs_pad.transpose(1, 2)).transpose(1, 2)
        xs_pad = F.relu(xs_pad)
        xs_pad = xs_pad.masked_fill(mask, 0.0)
    ...
    def g2forward(self, xs_pad, ilens):
        '''
                :param torch.Tensor xs_pad: batch of padded input sequences (B, Tmax, D)
                :param torch.Tensor ilens: batch of input length (B)
                :return: batch of padded hidden state sequences (B, Tmax // 4, 128)
        ...
        mask = ~make_pad_mask(ilens).unsqueeze(1).unsqueeze(-1)
        xs_pad = self.conv1_1_g2(xs_pad)
        xs_pad = self.norm1_1_g2(xs_pad.transpose(1, 2)).transpose(1, 2)
        xs_pad = F.relu(xs_pad)
        xs_pad = xs_pad.masked_fill(mask, 0.0)

        xs_pad = self.conv1_2_g2(xs_pad).masked_fill(mask, 0.0)
        xs_pad = self.norm1_2_g2(xs_pad.transpose(1, 2)).transpose(1, 2)
        xs_pad = F.relu(xs_pad)
        xs_pad = xs_pad.masked_fill(mask, 0.0)
        ...
        mask = ~make_pad_mask(ilens).unsqueeze(1).unsqueeze(-1)
        xs_pad = self.conv2_1_g2(xs_pad)
        xs_pad = self.norm2_1_g2(xs_pad.transpose(1, 2)).transpose(1, 2)
        xs_pad = F.relu(xs_pad)
        xs_pad = xs_pad.masked_fill(mask, 0.0)

        xs_pad = self.conv2_2_g2(xs_pad)
        xs_pad = self.norm2_2_g2(xs_pad.transpose(1, 2)).transpose(1, 2)
        xs_pad = F.relu(xs_pad)
        xs_pad = xs_pad.masked_fill(mask, 0.0)
    ...
    def forward(self, xs_pad, ilens):
        xs_pad_g1, mask_g1 = self.g1forward(xs_pad,ilens)
        xs_pad_g2, mask_g2 = self.g2forward(xs_pad, ilens)

        return xs_pad_g1, mask_g1